...
internal class MainPostAnalyticsBinder(
    ...
    fun createGridCard(card: DashboardCardType, account: String): View {
        ...
        if (isEditMode()) {
            ...
            PostCardBinder.applyEditModeState(activity, content, editMode = true)
    ...
    private fun postAnalyticsShell(label: String, body: String, post: PostSummary?): View =
        ...
        LinearLayout(activity).apply {
            ...
            setTag(R.id.post_card_open_url, post?.url.orEmpty())

            val opensPost = post?.url?.isNotBlank() == true && !isEditMode()
                ...
                post?.url?.takeIf { it.isNotBlank() }?.let { postUrl ->
                    ...
                    setOnClickListener {
                        ...
                        activity.startActivity(
                            ...
                            android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse(postUrl)),
            ...
            addView(TextView(activity).apply {
                ...
                tag = PostCardBinder.TAG_BODY

                text = when {
                    post == null -> body.ifBlank { "--" }
                    isEditMode() -> post.text.ifBlank { post.url }
                    else -> formattedPostText(post)
                }
    ...
    private fun tweetAuthorRow(post: PostSummary): View =
        ...
        LinearLayout(activity).apply {
            ...
            addView(ImageView(activity).apply {
                ...
                tag = PostCardBinder.TAG_AVATAR

                if (isEditMode()) {

                    PostCardBinder.applyAvatarGhost(activity, this)
