...
object TwidgetTheme {
    ...
    fun applyToApplication(context: Context) {
        AppCompatDelegate.setDefaultNightMode(nightMode(TwidgetStore.appThemeMode(context)))
    }
    ...
    /**
     * Applies the full accent theme in one shot (no overlay stacking).
    ...
    fun isDark(context: Context): Boolean {
        val mode = TwidgetStore.appThemeMode(context)
        return when (mode) {
    ...
    @ColorInt
    fun previewBackground(accent: String, dark: Boolean): Int =
        if (dark) darkPalettes.getValue(normalizeAccent(accent)).background
        else lightPalettes.getValue(normalizeAccent(accent)).background

    @ColorInt
    fun previewContainer(accent: String, dark: Boolean): Int =
