...
internal object WidgetPreviews {
        ...
        for (brief in listOf(false, true)) {
            ...
            val accepted = runCatching {
                manager.setWidgetPreview(provider, AppWidgetProviderInfo.WIDGET_CATEGORY_HOME_SCREEN,
                    views(context, settings, dark, brief))
            }.getOrDefault(false)
    ...
    @RequiresApi(35)
    internal fun views(context: Context, defaults: TwidgetWidgetSettings, dark: Boolean, brief: Boolean): RemoteViews {
        ...
        return RemoteViews(listOf(162 to 76, 352 to 76, 162 to 176, 352 to 176).associate { (w, h) ->
            val bitmap = if (brief) BriefWidgetArtworkRenderer.render(context,
                ...
                TwidgetWidget.layoutModeForAosp(w, h), dark, drawBackground = true)
            SizeF(w.toFloat(), h.toFloat()) to RemoteViews(context.packageName, R.layout.widget_generated_preview).apply {
                setImageViewBitmap(R.id.widget_preview_artwork, bitmap)
            }
        })
    }
}
