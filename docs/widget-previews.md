...
The follower layouts follow the Figma home widgets at 162×76, 352×76, 162×176 and 352×176 dp. Large-card headings grow or shrink to fit the actual launcher width and height, reserving space for the footer and its clearance. Fitting uses visible glyph bounds, including accents and descenders. All lines share one baseline advance derived from the paragraph’s ascent/descent envelope, so individual letters never alter the line spacing; footer logos stay at 12 dp independently of handle text. One UI compact widgets have pill-shaped surfaces.
...
The same scroll view also disables overscroll stretch: Android promotes stretched content into a hardware layer, hiding the wallpaper opening during the edge pull and rebound. Ordinary scrolling and flinging remain enabled. `wallpaperRemainsVisibleWhilePullingPastTop` sends a touch gesture through the activity and samples the wallpaper gutter while held, after release, and once settled; programmatic `scrollTo` alone does not exercise this effect.
